/**
 * Technical Analysis & High-Accuracy Signal Strategy Engine
 * Evaluates Multiple Confluence Factors (EMA Fast/Slow cross, RSI overbought/oversold & momentum, MACD, Volume trend)
 */
...
function analyzeMarket(candles) {
  // candles format: [[time, open, high, low, close, volume], ...]
  if (!candles || candles.length < 30) {
    return { signal: 'NEUTRAL', confidence: 0, reason: 'Insufficient candle data' };
  }
  ...
  // Factor 1: EMA Trend & Crossover
  if (latestEma9 > latestEma21) {
    buyScore += 30;
    if (prevEma9 <= prevEma21) {
      buyScore += 20; // Fresh Bullish Golden Cross
      reasons.push('Golden Cross (EMA9 crossed above EMA21)');
    } else {
      reasons.push('Bullish EMA alignment (EMA9 > EMA21)');
    }
  } else if (latestEma9 < latestEma21) {
    sellScore += 30;
    if (prevEma9 >= prevEma21) {
      sellScore += 20; // Fresh Bearish Death Cross
      reasons.push('Death Cross (EMA9 crossed below EMA21)');
    } else {
      reasons.push('Bearish EMA alignment (EMA9 < EMA21)');
    }
  }

  // Factor 2: RSI Momentum & Reversals
  if (latestRSI > 50 && latestRSI < 68) {
    buyScore += 25;
    reasons.push(`RSI Bullish Momentum (${latestRSI.toFixed(1)})`);
  } else if (latestRSI < 30) {
    buyScore += 30;
    reasons.push(`RSI Oversold Bounce Setup (${latestRSI.toFixed(1)})`);
  } else if (latestRSI < 50 && latestRSI > 32) {
    sellScore += 25;
    reasons.push(`RSI Bearish Momentum (${latestRSI.toFixed(1)})`);
  } else if (latestRSI > 70) {
    sellScore += 30;
    reasons.push(`RSI Overbought Pullback Setup (${latestRSI.toFixed(1)})`);
  }

  // Factor 3: MACD Confluence
  if (macdData) {
  ...
  if (volumeMultiplier >= 1.25) {
    ...
    reasons.push(`High Volume Spike (${volumeMultiplier.toFixed(1)}x avg)`);
  }
  ...
  if (buyScore >= 65 && buyScore > sellScore + 15) {
    finalSignal = 'BUY';
    confidence = Math.min(96, buyScore);
  } else if (sellScore >= 65 && sellScore > buyScore + 15) {
    finalSignal = 'SELL';
    confidence = Math.min(96, sellScore);
  } else {
